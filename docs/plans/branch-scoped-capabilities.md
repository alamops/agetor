...
- **Consumers of the endpoint:** the webview only (`PromptComposer.useAgentCapabilities`, hoisted in RunPanel). `src/cli/api-client.ts:305` exposes `agentDiscovery(agent, workdir, branch)` but no CLI/TUI code calls it. `listAvailableCommands` is called directly only by `commands.test.ts`.
...
- Assumption: no CLI/TUI surface consumes `/agent-discovery` (grep found only the unused `api-client.agentDiscovery`), so no CLI parity task.
...
| CLI `agetor add` capability listing at a ref | **out of scope** — no CLI caller exists today |
