...
- **Panel structure:** the "task details modal" is a right-hand `<aside>` slide-over (`RunPanel.tsx:170`), not a Dialog. `RunPanelBody` renders, top to bottom: `<header>` (`:860`), `<FileMentions>` (`:921`; removed 2026-08-20 — see docs/plans/remove-files-mentioned.md), `<TaskDetails>` (`:927`), `<RunsList>` (`:936`), `<TerminalsSection>` (`:938`), `<SubagentTabs>` (`:940-949`), the event scroll region (`:951-999`), then the composer (`:1009`).
