...
- **UI surfacing (added by owner request after review)**: `WorktreeInfo.heldByBackgroundAgents`, populated from one grouped `subagents.runningCountsByTask()` query per listing (which also replaces the per-row `hasRunning` call in the `"inactive"` condition). The Worktrees page shows an "agents working" indicator when the main run is idle but agents are held, and the archive-confirm warning now fires for held tasks too — closing the pre-existing gap where archiving would silently stop still-running background agents.
