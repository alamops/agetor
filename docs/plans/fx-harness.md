...
- **T1 types** — owns `src/shared/types.ts`: `AgentKind` + `fx` entries in `HARNESS_TEMPLATES` (`fx-additional`), `DEFAULT_MODEL` (`zai/glm-5.2-fast`, *(superseded — see fx-0.0.6-compat.md: now `moonshotai/kimi-k3`)*), `DEFAULT_EFFORT` (symmetry value), `CODE_PLAN_MODE` (`{code:"yolo", plan:"ask"}`), `MODEL_EFFORT_SUPPORT` (all `[]`), `MODEL_MODE_DENY` (`{}`), `AGENT_OPTIONS` (5 models, 3 modes yolo/auto/ask with labels+hints), `Run.fxSessionId` + doc comment, `Harness.home` doc paragraph (HOME-override isolation).
