...
| 4 | Watchdog on an idle input box: card / ignore / settle? | Settle after the same 60 s + 3 stable ticks. "Never fires while idle at the input box" is the fallback's own acceptance criterion; ignoring would recreate the pre-#185 stuck-running state. | `unknown-claude-prompts-fallback.md` §1; spike idle captures | Med-High |
...
| 6 | `/model` picker: confirm with Enter (sets global default) or `s` (session only)? | Enter — mirrors the TUI and `/model <id>`'s "saved as your default" semantics. One-line flip in `dismissTmuxPrompt` if undesired. | Spike footer text | Medium |
