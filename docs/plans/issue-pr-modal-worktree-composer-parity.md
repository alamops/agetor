...
## 11. Follow-up (owner: "fix these" — the three §10 open items)

| Task | Owns | Scope |
| --- | --- | --- |
| A1 Type picker | `kanban/TaskTypePicker.tsx` (new), `NewTaskForm.tsx`, `CreateTaskFromIssueDialog.tsx`, `shared/issue-task.ts` (+test), `cli/commands/add.ts` | Extract the panel's Type grid into `TaskTypePicker`; pure `inferTaskTypeFromLabels(labels)` (bug/defect/regression → `bug`; spike/research/investigation/exploration → `spike`; else `task`); the issue modal seeds its type from the issue's labels (until the user picks), drives `useWorktreeOptions({ taskType })` so the branch prefix follows, and sends `taskType`; `agetor add --issue` defaults `--type` the same way. |
| A2 RunPanel composer | `RunPanel.tsx`, `PromptComposer.tsx` (+ whatever the recon says) | Migrate the send box onto the shared hooks and — if it can be expressed without visual change — the component (placement above, inline refs, history-picker slot, Enter-to-send passthrough). |
| A3a pick seam | `bun/server.ts`, `e2e/fixtures.ts`, `kanban/ReferencesPicker.tsx` (test ids) | `AGETOR_FAKE_PICK_REFS_DIR`: `/refs/pick` returns that directory's files (mode `files`) or the directory (mode `folder`) instead of opening a native panel; fixtures create `<dataDir>/fake-picks`, set the env and expose `backend.fakePickDir`. |
