...
export type AgentKind = "claude-code" | "codex" | "kimi";
...
export const HARNESS_TEMPLATES: HarnessTemplate[] = [
  ...
  {
    id: "kimi-additional",
    label: "Additional Kimi Code",
    description:
      "Another Kimi Code harness with its own KIMI_CODE_HOME so login and history are isolated from the built-in.",
    kind: "kimi",
    suggestedHarnessId: "kimi-2",
    home: "{dataDir}/harnesses/kimi-2",
    bin: null,
    env: {},
  },
];
...
export const DEFAULT_EFFORT: Record<AgentKind, string> = {
  "claude-code": "high",
  "codex": "high",
  // kimi declines the effort flag entirely (see MODEL_EFFORT_SUPPORT below) —
  // this default is never surfaced in the picker, kept only for symmetry.
  "kimi": "high",
};
...
export const CODE_PLAN_MODE: Record<AgentKind, { code: string; plan: string }> = {
  ...
  // kimi has no first-class plan mode either — same "ask" (read-only tools)
  // posture as codex.
  "kimi": { code: "auto", plan: "ask" },
};
...
export const MODEL_EFFORT_SUPPORT: Record<AgentKind, Record<string, string[]>> = {
  ...
  // kimi-cli only exposes a boolean `--thinking` switch, not a graded effort
  // parameter — every curated model declines the effort flag entirely (empty
  // list), same treatment as Haiku 4.5 above. The picker collapses to nothing
  // and `buildCommand` never emits an effort flag for kimi.
  kimi: {
    "kimi-k2.7-code": [],
    "kimi-k2.6": [],
    "kimi-k2.5": [],
...
export const AGENT_OPTIONS: Record<AgentKind, AgentOptions> = {
  ...
  kimi: {
    models: [
      { id: "kimi-k2.7-code", label: "Kimi K2.7 Code", hint: "Recommended default — Moonshot's coding-tuned flagship." },
      { id: "kimi-k2.6", label: "Kimi K2.6", hint: "Prior generation." },
      { id: "kimi-k2.5", label: "Kimi K2.5", hint: "Earlier generation." },
    ],
    modes: [
      { id: "auto", label: "Auto (full auto-approve)", hint: "Print mode auto-approves every tool call." },
      { id: "ask", label: "Read-only", hint: "Inspect only — read-only tool set (--plan)." },
    ],
    // kimi-cli has no graded effort parameter — see MODEL_EFFORT_SUPPORT.
    // Kept as EFFORT_OPTIONS for symmetry with the other kinds so a stray
    // stored id still resolves to a label instead of a bare string; the
    // picker collapses to empty via supportedEfforts() regardless.
    efforts: EFFORT_OPTIONS,
  },
};
...
export interface Run {
  ...
  /**
   * Kimi Code's own per-turn session id — a `crypto.randomUUID()` pre-
   * generated by the driver at first spawn (kimi's `--session <id>` both
   * resumes and creates, so agetor mints the id rather than parsing one out
   * of stdout). Reported via the driver's `onSessionId` callback immediately
   * on spawn and persisted here; follow-up turns pass the same id via
   * `--session`. NULL for claude-code/codex and pre-migration rows.
   */
