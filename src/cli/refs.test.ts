...
import { resolveRefs, missingRefs } from "./refs.ts";
...
test("missingRefs flags only the paths that don't exist", () => {
  const dir = mkdtempSync(path.join(tmpdir(), "agetor-refs-"));
  const file = path.join(dir, "a.txt");
  writeFileSync(file, "x");
  const refs = resolveRefs([file, path.join(dir, "gone.png")]);
  expect(missingRefs(refs).map((r) => r.path)).toEqual([path.join(dir, "gone.png")]);
  rmSync(dir, { recursive: true, force: true });
});
