...
export async function cmdHarness(args: string[], flags: Flags): Promise<void> {
  ...
  switch (sub) {
    ...
    case "shell": {
      ...
      // Only claude's login command is certain (`claude /login`); kimi's is
      // doc-verified as `kimi login`. Keep the codex hint generic rather than
      // assert a command we haven't verified.
      const loginHint =
        kind === "claude-code"
          ? `run ${c.cyan(`${launch} /login`)} to authenticate`
          : kind === "kimi"
            ? `run ${c.cyan(launch)} and use ${c.cyan("kimi login")} to authenticate`
