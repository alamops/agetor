...
- **Agents.** Save a harness + model + effort + mode + free-text instructions + a skills list as a named, reusable **Agent** in Settings, then pick it on task launch instead of choosing each field by hand. Its instructions are injected into the launch prompt; a task follows the live profile until its first run, then keeps exactly what it launched with even if the profile changes later.
